...
int strToDouble(const std::string &inputString, long &d_value) {
	double doubleValue;
	std::istringstream inStr(inputString);
	inStr >> d_value;
	if ((inStr.rdstate() & std::ios_base::failbit) > 0) return 1;
...
int main() {
	...
	if (strToLong(str,l) != 0){
		std::cerr << "ERROR: strToLong faild!" << std::endl;
	}
	if (strToDouble(str,l) != 0){
		std::cerr << "ERROR: strToDouble faild!" << std::endl;
	}
